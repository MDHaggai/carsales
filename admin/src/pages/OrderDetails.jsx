...
const ShippingTracker = ({ order, onLocationUpdate, token }) => {
  ...
  const renderRouteInfo = () => {
    if (!routeInfo || typeof routeInfo.distance !== 'number') return null;
    const { distance, duration } = routeInfo;
    const { progress, remainingTime } = persistedData;
      ...
      <div className="mb-4 p-4 bg-gray-50 rounded-lg">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div>
            ...
            <p className="font-medium text-blue-600">{formatTime(remainingTime)}</p>
          </div>
          <div>
  ...
  // Update the map section in ShippingTracker
        ...
        {origin && destination && (
          <RouteCalculator
            ...
            onRouteCalculated={(newRouteInfo) => {
              console.log('Route info updated:', newRouteInfo);
  ...
  const renderMovementControls = () => {
    ...
    return (
      <div className="flex gap-4 justify-center">
        {movementStatus === MOVEMENT_STATUSES.PAUSED || movementStatus === MOVEMENT_STATUSES.NOT_STARTED ? (
          <button
            onClick={async () => {
              try {
                await updateShippingInfo({
                  movementStatus: MOVEMENT_STATUSES.ON_TRANSIT,
                  startTime: new Date(),
                  estimatedDuration: routeInfo.duration,
                  remainingTime: routeInfo.duration
                });
                setMovementStatus(MOVEMENT_STATUSES.ON_TRANSIT);
            ...
            disabled={isUpdating}
          >
            {movementStatus === MOVEMENT_STATUSES.NOT_STARTED ? 'Start Movement' : 'Resume Movement'}
          </button>
        ) : (
          <button
            onClick={async () => {
              try {
                await updateShippingInfo({
                  movementStatus: MOVEMENT_STATUSES.PAUSED,
                  lastPausedAt: new Date()
                });
                setMovementStatus(MOVEMENT_STATUSES.PAUSED);
      ...
      try {
        ...
        if (response.data.success) {
          ...
          setPersistedData(prev => ({
            ...
            currentLocation: shipping.routeMarker?.position || shipping.currentLocation
          }));

          // Update route marker position
          if (mapInstanceRef.current && shipping.routeMarker?.position) {
            const { lat, lng } = shipping.routeMarker.position;
